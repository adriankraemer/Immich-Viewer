...
struct SwipeGestureView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> UIView {
        ...
        debugLog("SwipeGestureView: Creating UIView with user interaction enabled")
        ...
        debugLog("SwipeGestureView: Added LEFT gesture recognizer")
        ...
        debugLog("SwipeGestureView: Added RIGHT gesture recognizer")
        ...
        debugLog("SwipeGestureView: Added LEFT swipe gesture recognizer")
        
        let rightSwipe = UISwipeGestureRecognizer(target: context.coordinator, action: #selector(Coordinator.handleRight))
        rightSwipe.direction = .right
        view.addGestureRecognizer(rightSwipe)
        debugLog("SwipeGestureView: Added RIGHT swipe gesture recognizer")
        ...
        debugLog("SwipeGestureView: Added test tap gesture recognizer")
        
        debugLog("SwipeGestureView: All gesture recognizers added for tvOS navigation")
    ...
    class Coordinator: NSObject {
        ...
        init(onSwipeLeft: @escaping () -> Void, onSwipeRight: @escaping () -> Void) {
            ...
            debugLog("SwipeGestureView.Coordinator: Initialized with navigation callbacks")
        }
        
        @objc func handleLeft(_ gesture: UIGestureRecognizer) {
            debugLog("SwipeGestureView: LEFT gesture detected (type: \(type(of: gesture))) - navigating to previous photo")
            onSwipeRight()
        }
        
        @objc func handleRight(_ gesture: UIGestureRecognizer) {
            debugLog("SwipeGestureView: RIGHT gesture detected (type: \(type(of: gesture))) - navigating to next photo")
            onSwipeLeft()
        }
        
        @objc func testTap(_ gesture: UITapGestureRecognizer) {
            debugLog("SwipeGestureView: Test tap detected - view is receiving touch events!")
        }
    }
} 